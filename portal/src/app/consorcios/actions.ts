...
function formToConsorcio(formData: FormData) {
  ...
  return {
    ...
    uf_retiro_residuos: formData.get("uf_retiro_residuos") ? Number(formData.get("uf_retiro_residuos")) : null,
...
export async function createConsorcio(formData: FormData) {
  ...
  await queryOne(
    `INSERT INTO app.consorcios
       (nombre, direccion, cuit, codigo_postal, suterh_key, cant_uf, uf_retiro_residuos,
...
export async function updateConsorcio(formData: FormData) {
  ...
  await queryOne(
    `UPDATE app.consorcios SET
       ...
       suterh_key = $4, cant_uf = $5, uf_retiro_residuos = $6, categoria_edificio = $7, banco = $8,
       tiene_cochera = $9, tiene_movimiento_coches = $10, tiene_jardin = $11,
       zona_desfavorable = $12, tiene_pileta = $13, tiene_caldera = $14,
       tiene_ascensor = $15, tiene_agua_caliente_central = $16,
       tiene_calefaccion_central = $17, tiene_aire_acondicionado_central = $18,
       tiene_grupo_electrogeno = $19, tiene_seguridad_centralizada = $20,
       tiene_compactador = $21, tiene_montacargas = $22,
