...
function PieChart({
  ...
  return (
    <div className="flex items-center gap-6 print:gap-2">
      <svg width="140" height="140" viewBox="0 0 140 140" className="flex-shrink-0 print:w-[60px] print:h-[60px]">
        {paths.map((p, i) => (
          <path key={i} d={p.d} fill={p.color} stroke="white" strokeWidth="1.5" />
        ))}
      </svg>
      <table className="text-xs print:text-[8px] w-full">
        <tbody>
          {paths.map((p, i) => (
            <tr key={i}>
...
export default async function ReciboPage({
  ...
  function inferUnidadValor(
    concepto: string,
    importe: number,
    opts: { anios?: number; totalUnidades?: number | null; ufRetiroResiduos?: number | null; cantUf?: number | null } = {}
    ...
    if (c.includes("retiro de residuos")) {
      ...
      const unidad = uf ? `${uf} unidades` : "—";
      const unitVal = uf && uf > 0 ? fmt(Math.round((i / uf) * 100) / 100) : fmt(i);
    ...
    const pctMap: Record<string, number> = {
      ...
      suterh: 2, "caja protección": 1, fateryh: 1, "seguro vitalicio": 0.75, "seguro colectivo de vida": 0.75,
    };
  ...
  const pieSlices = [
    ...
    { label: "Sindical/Convencional", value: baseOtras * (pctSuterh + pctFateryh + pctSeracarh) + fateryhFijo, color: PIE_COLORS[5] },
  ];
      ...
      <div className="max-w-4xl mx-auto p-6 print:p-0 print:space-y-0">
        ...
        <div className="border-2 border-gray-800 mb-0 print:break-inside-avoid">
          ...
          <div className="grid grid-cols-2 divide-x divide-gray-300">
            {/* Left: employer */}
            <div className="p-3 print:p-1 text-xs print:text-[8px] space-y-0.5">
            ...
            <div className="p-3 print:p-1 text-xs print:text-[8px] grid grid-cols-2 gap-2">
              <div>
        ...
        <div className="border-x-2 border-t-2 border-gray-800 mt-0">
          ...
          <div className="flex justify-between items-center bg-gray-200 px-3 py-1.5 print:py-[1px] border-b border-gray-400">
          ...
          <table className="w-full text-xs print:text-[8px] border-collapse table-fixed">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="text-right p-2 print:p-0.5 font-semibold text-gray-600 w-[14%]">Unidad</th>
                <th className="text-right p-2 print:p-0.5 font-semibold text-gray-600 w-[20%]">Base</th>
                <th className="text-right p-2 print:p-0.5 font-semibold text-gray-600 w-[20%]">Monto</th>
              </tr>
            </thead>
            <tbody>
              {patronalRows.map((r, i) => (
                <tr key={i} className="border-b border-gray-100">
                  ...
                  <td className="p-1.5 print:p-[1px] text-right font-medium text-gray-900">${fmt(r.importe)}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="flex justify-between items-center bg-gray-100 px-3 py-1.5 print:py-[1px] border-t border-gray-300">
          ...
          {/* SUELDO BRUTO header */}
          <div className="flex justify-between items-center bg-gray-200 px-3 py-1.5 print:py-[1px] border-t border-gray-400">
          ...
          <table className="w-full text-xs print:text-[8px] border-collapse table-fixed">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="text-right p-2 print:p-0.5 font-semibold text-gray-600 w-[10%]">Unidad / Alíc.</th>
                <th className="text-right p-2 print:p-0.5 font-semibold text-gray-600 w-[12%]">Valor Unit.</th>
                <th className="text-right p-2 print:p-0.5 font-semibold text-gray-600 w-[14%]">Remunerativo</th>
                <th className="text-right p-2 print:p-0.5 font-semibold text-gray-600 w-[14%]">No Remuner.</th>
                <th className="text-right p-2 print:p-0.5 font-semibold text-gray-600 w-[14%]">Descuentos</th>
              </tr>
            </thead>
            <tbody>
              {haberes.map((h: any, i: number) => {
                ...
                const { unidad, valorUnit } = inferUnidadValor(h.concepto, Math.abs(val), { anios: liq.antiguedad_anios ?? 0, totalUnidades: liq.consorcio_total_unidades, ufRetiroResiduos: liq.uf_retiro_residuos, cantUf: liq.cant_uf });
                return (
                  <tr key={i} className="border-b border-gray-100">
                    ...
                    <td className="p-1.5 print:p-[1px] text-right font-medium text-blue-700">
              ...
              {descuentos.map((d: any, i: number) => {
                ...
                return (
                  <tr key={i} className="border-b border-gray-100">
                    ...
                    <td className="p-1.5 print:p-[1px]" />
                    <td className="p-1.5 print:p-[1px]" />
                    <td className="p-1.5 print:p-[1px] text-right font-medium text-red-700">${fmt(Number(d.importe))}</td>
                  </tr>
                );
              })}
              ...
              {redondeo > 0.005 && (
                <tr className="border-b border-gray-100 text-gray-400 italic">
                  <td className="p-1.5 print:p-[1px] pl-2">Redondeo</td>
                  <td className="p-1.5 print:p-[1px] text-right">—</td>
                  <td className="p-1.5 print:p-[1px] text-right">—</td>
                  <td className="p-1.5 print:p-[1px]" />
                  <td className="p-1.5 print:p-[1px] text-right font-medium text-blue-700">${fmt(redondeo)}</td>
                  <td className="p-1.5 print:p-[1px]" />
                </tr>
              )}
            </tbody>
            <tfoot>
              ...
              <tr className="border-t border-gray-300 bg-gray-50 text-xs">
                <td className="p-2 print:p-0.5 font-semibold text-gray-700" colSpan={3}>COMPOSICIÓN SALARIAL</td>
                <td className="p-2 print:p-0.5 text-right text-gray-700">
                  Rem.: ${fmt(haberes.filter((h: any) => !NO_REMUNERATIVOS.has(h.concepto)).reduce((s: number, h: any) => s + r2(Number(h.importe)), 0))}
                </td>
                <td className="p-2 print:p-0.5 text-right text-blue-700">
                  {(() => {
                ...
                <td className="p-2 print:p-0.5 text-right text-red-700">
